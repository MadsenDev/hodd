...
function AndroidDevice({
  ...
  bg, statusDark, navDark, contentStyle,
}) {
  const deviceBg = bg || (dark ? '#1d1b20' : MD_C.surface);
  const sDark = statusDark !== undefined ? statusDark : dark;
  const nDark = navDark !== undefined ? navDark : dark;
  return (
    <div style={{
      width, height, borderRadius: 44, overflow: 'hidden',
      background: deviceBg,
      border: `10px solid #0a0a0c`,
      boxShadow: '0 40px 100px -20px rgba(0,0,0,0.6), 0 0 0 1.5px rgba(255,255,255,0.06)',
      display: 'flex', flexDirection: 'column', boxSizing: 'border-box',
    }}>
      <AndroidStatusBar dark={sDark} />
